...
async def checkUserExists(bot: commands.Bot, user_id: int, mention: str, emoji: str) -> models.CompositeAction:
        ...
        async with bot.db.acquire() as connection:
            async with connection.transaction():
                try:
                    ...
                    return models.CompositeAction(actions, False)
    else:
        return models.CompositeAction(actions, True)
...
def prepareTimestamp(timestamp: datetime, tz: dt.tzinfo, str: bool=True) -> Union[str, datetime]:
...
async def id_generator(size=6, chars=string.digits):
