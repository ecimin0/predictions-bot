        ...
        try:
            ...
            for fix in scorable_fixtures:
                try:       
                    ...
                    goals = [event for event in fixture_info.get("events") if event.get("type") == "Goal" and event.get("team").get("id") == self.bot.main_team and "Penalty" not in event.get("detail")]
                    new_goals = []
                    for goal in goals:
